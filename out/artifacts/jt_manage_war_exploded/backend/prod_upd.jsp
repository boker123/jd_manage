...
<script type="text/javascript">
	/* document loading events,when document has been load executed js*/
	$(function () {
		var category = $("select[name=category]").attr("val");
		$("option:contains('"+category+"')").attr("selected", true);
	});
</script>
</head>
<body>
	<h2>修改商品信息</h2>
	<hr/>
	<form action="${app}/ProdUpdateServlet" method="POST">
		...
		<table border="1">
			...
			<tr>
				<td>商品种类</td>
				<td>
					<select name="category" val="${prod.category}">
						<option>手机数码</option>
						<option>电脑平板</option>
						<option>家用电器</option>
						<option>汽车用品</option>
